...
<div class="container">
				...
				<table class="table" style="margin-bottom: 20px;border:1px solid rgb(222, 222, 222);" id="table-orders-product">
					<thead>
						<tr>
							...
							<td class="col-md-2 col-sm-2 col-xs-2 text-center">留言/标注</td>
						</tr>
					</thead>
					...
					<tr>
						...
						<td class="text-center col-md-1 col-sm-1 col-xs-2 border-right"><span><?php echo $order['status_name']; ?></span><a href="<?php echo site_url('order/orders/order_info?order_id=').$order['order_id'];?>">订单详情</a></td>
	...
	<script>
		...
		function del_order(id){
			$.ajax({
				url: '<?php echo site_url();?>/order/orders/del_order.html',
				type: 'post',
				dataType: 'json',
				data: {order_id:id},
				beforeSend: function()

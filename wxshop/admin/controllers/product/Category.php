...
class Category extends MY_Controller {
	...
	protected function get_list(){
		...
		$data['delete']					=site_url('product/category/delete');
		...
		$config['base_url'] 			= site_url('product/category');
		$config['num_links'] 			= 2;
	...
	protected function get_form()
	{
		...
		if($this->input->get('category_id')){
			$data['action'] = site_url().'/product/category/edit.html?category_id='.$this->input->get('category_id');
		}else{
			$data['action'] = site_url().'/product/category/add.html';
		}

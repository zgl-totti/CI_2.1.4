...
class Product extends MY_Controller {
	...
	public function __construct() {
		...
		$this->load->model(array('common/module_model', 'common/product_model'));
	}
	...
	public function edit(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->check_modify()){
			...
			redirect(site_url('extension_config/module/product'));
		}
	...
	public function get_form()
	{
		...
		if($this->input->get('module_id') != NULL){
			$data['action']=site_url('extension_config/module/product/edit?module_id=').$this->input->get('module_id');
			//查数据
			$module=$this->module_model->get_modules_for_module_id($this->input->get('module_id'));
			//var_dump($data['module']);
			
		}else{
			$data['action']=site_url('extension_config/module/product/add');
		}

...
class Qq extends MY_Controller {
	...
	public function __construct() {
		...
		if(!$this->user->hasPermission('access', 'admin/extension_config/sign_in_with/qq')){
	...
	public function add(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->validateForm()){
			$this->sign_in_with_model->add($this->input->post(), 'sign_in_with', 'qq');
	...
	{
		$extension_setting=$this->sign_in_with_model->get_sign_with('sign_in_with', 'qq');
		...
		$this->load->view('theme/default/template/extension_config/sign_in_with/qq',$data);
	}
	
	//验证表单
	private function validateForm(){
		if (!$this->user->hasPermission('modify', 'admin/extension_config/sign_in_with/qq')) {

...
class Github extends MY_Controller {
	...
	public function __construct() {
		...
		if(!$this->user->hasPermission('access', 'admin/extension_config/sign_in_with/github')){
	...
	{
		$this->document->setTitle('github登陆');
	...
	public function add(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->validateForm()){
			$this->sign_in_with_model->add($this->input->post(), 'sign_in_with', 'github');
	...
	{
		$extension_setting=$this->sign_in_with_model->get_sign_with('sign_in_with', 'github');
		...
		$this->load->view('theme/default/template/extension_config/sign_in_with/github',$data);
	}
	
	//验证表单
	private function validateForm(){
		if (!$this->user->hasPermission('modify', 'admin/extension_config/sign_in_with/github')) {

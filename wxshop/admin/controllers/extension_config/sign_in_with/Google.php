...
class Google extends MY_Controller {
	...
	public function __construct() {
		...
		if(!$this->user->hasPermission('access', 'admin/extension_config/sign_in_with/google')){
	...
	{
		$this->document->setTitle('google登陆');
	...
	{
		$extension_setting=$this->sign_in_with_model->get_sign_with('sign_in_with', 'google');
		...
		$this->load->view('theme/default/template/extension_config/sign_in_with/google',$data);
	}
	
	//验证表单
	private function validateForm(){
		if (!$this->user->hasPermission('modify', 'admin/extension_config/sign_in_with/google')) {

...
class Orders extends MY_Controller {
	...
	public function index()
	{
		...
		// 分页
		$config ['base_url'] = site_url ('order/orders').(($this->input->get('order_status') != NULL) ? '?order_status='.$this->input->get('order_status') : '');
		$config ['num_links'] = 2;
	...
	public function order_info(){
		...
		$data['top']=$this->header->top();
		$data['footer']=$this->footer->index();
		$this->load->view('theme/default/template/order/order_info',$data);
	}

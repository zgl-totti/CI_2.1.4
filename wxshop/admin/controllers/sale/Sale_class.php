...
class Sale_class extends MY_Controller {
	...
	public function get_list(){
		...
		//分页
		$config['base_url'] 			= site_url('sale/sale_class');
		$config['num_links'] 			= 2;
		...
		$data['delete']					=site_url('sale/sale_class/delete');
	...
	public function get_from()
	{
		...
		if($this->input->get('sale_class_id')){
			$data['action']=site_url('sale/sale_class/edit?sale_class_id=').$this->input->get('sale_class_id');
		}else{
			$data['action']=site_url('sale/sale_class/add');
		}

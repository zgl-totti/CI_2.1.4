...
class Tax_class extends MY_Controller {
	...
	public function add()
	{
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->validate_form()){
			...
			redirect(site_url('localisation/tax_class'));
		}
	...
	public function get_form()
	{
		...
		if($this->input->get('tax_class_id')){
			$data['action']					=site_url('localisation/tax_class/edit?tax_class_id=').$this->input->get('tax_class_id');
		}else{
			$data['action']					=site_url('localisation/tax_class/add');
		}
	...
	public function get_list()
	{
		...
		//分页
		$config['base_url'] 			= site_url('localisation/tax_class');
		$config['num_links'] 			= 2;
		...
		$data['delete']					=site_url('localisation/tax_class/delete');

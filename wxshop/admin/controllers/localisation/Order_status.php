...
class Order_status extends MY_Controller {
	...
	public function add()
	{
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->validate_form()){
			...
			redirect(site_url('localisation/order_status'));
		}
	...
	public function get_form()
	{
		...
		if($this->input->get('order_status_id')){
			$data['action']					=site_url('localisation/order_status/edit?order_status_id=').$this->input->get('order_status_id');
		}else{
			$data['action']					=site_url('localisation/order_status/add');
		}
	...
	public function get_list()
	{
		...
		//分页
		$config['base_url'] 			= site_url('localisation/order_status');
		$config['num_links'] 			= 2;
		...
		$data['delete']					=site_url('localisation/order_status/delete');

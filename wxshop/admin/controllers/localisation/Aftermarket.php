...
class Aftermarket extends MY_Controller {
	...
	public function add()
	{
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->validate_form()){
			...
			redirect(site_url('localisation/aftermarket'));
		}
	...
	public function get_form()
	{
		...
		if($this->input->get('aftermarket_id')){
			$data['action']					=site_url('localisation/aftermarket/edit?aftermarket_id').$this->input->get('aftermarket_id');
		}else{
			$data['action']					=site_url('localisation/aftermarket/add');
		}
	...
	public function get_list()
	{
		...
		//分页
		$config['base_url'] 			= site_url('localisation/aftermarket');
		$config['num_links'] 			= 2;
		...
		$data['delete']					=site_url('localisation/aftermarket/delete');

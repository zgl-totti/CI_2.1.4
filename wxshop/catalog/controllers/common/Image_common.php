...
class Image_common extends CI_Common {
	...
	public function resize($filename, $width, $height, $master_dim='h') {
		...
		if (!is_file(IMGPATH . $new_image) || (filectime(IMGPATH . $old_image) > filectime(IMGPATH . $new_image))) {
			...
			if ($width_orig != $width || $height_orig != $height) {
				...
				//$new_image=$this->crop($new_image, $width, $height);
			} else {
	...
	function crop($new_img,$width,$height){
		$old_image=IMGPATH.$new_img;
		$image_size=getimagesize($old_image);
		$old_weight=$image_size[0];
		$old_height=$image_size[1];

		if($old_weight != $width || $old_height != $height){
			$config1['source_image']			 	= $old_image;
			$config1['maintain_ratio'] 				= FALSE;
			$config1['width'] 	 		 	 		= $width;
			$config1['height'] 	 		 		 	= $height;
			//$config1['y_axis']						= $old_height - $height;
			$config1['y_axis']						= 100;
			//$config1['x_axis']						= $old_weight - $width;
			$config1['x_axis']						= 100;

...
class FileManager extends CI_Controller {
	...
	public function index() {
		...
		$data['parent'] = site_url().'/common/filemanager?token=' . $_SESSION['token'] . $url;
		...
		$data['refresh'] = site_url().'/common/filemanager?token=' . $_SESSION['token'] . $url;
		...
		$config['base_url'] 					= site_url().'/common/filemanager?token='.$_SESSION['token'].$url;
		$config['page_query_string']			= TRUE;
		$config['query_string_segment']			= 'page';
		//样式
		$config['full_tag_open'] 				= '<td colspan="2"><nav class="text-left" style="float:left"><ul class="pagination">';
		$config['full_tag_close'] 				= '</ul></nav></td><td colspan="2"><nav><ul class="pagination navbar-right"><li><a>'. lang_line('text_altogether') . $image_total . lang_line('text_price') . '</a></li></ul></nav></td>';
		...
		if (!$json) {
			...
			if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
			    if (preg_match("/[\x7f-\xff]/", $folder)) {
				  $json['error'] = $this->lang->line('error_windows');
				 } 
			}
		}

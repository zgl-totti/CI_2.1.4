...
class Freight extends MY_Controller {
	...
	public function __construct() {
		...
		if(!$this->user->hasPermission('access', 'sale/product/freight')){
	...
	public function get_form()
	{
		...
		if($this->input->get('freight_id')){
			$data['freight_action']	=site_url('product/freight/edit_freight?freight_id=').$this->input->get('freight_id');
		}else{
			$data['freight_action']	=site_url('product/freight/add_freight');
		}
		
		if($this->input->get('freight_template_id')){
			$data['freight_template_action']	=site_url('product/freight/edit_freight_template?freight_template_id=').$this->input->get('freight_template_id');
		}else{
			$data['freight_template_action']	=site_url('product/freight/add_freight_template');
		}
	...
	public function get_list()
	{
		...
		//分页
		$config['base_url'] 			= site_url('product/freight');
		$config['num_links'] 			= 2;
		...
		//分页
		$config1['base_url'] 			= site_url('product/freight');
		$config1['num_links'] 			= 2;
		...
		$data['freight_delete']		=site_url('product/freight/delete_freight');
		$data['freight_template_delete']	=site_url('product/freight/delete_freight_template');

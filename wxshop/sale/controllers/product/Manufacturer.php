...
class Manufacturer extends MY_Controller {
	...
	public function __construct() {
		...
		if(!$this->user->hasPermission('access', 'sale/product/manufacturer')){
	...
	public function get_form ()
	{
		...
		if($this->input->get('manufacturer_id')){
			$data['action']=site_url('product/manufacturer/edit?manufacturer_id=').$this->input->get('manufacturer_id');
		}else{
			$data['action']=site_url('product/manufacturer/add');
		}
	...
	public function get_list()
	{
		...
		$data['delete']=site_url('product/manufacturer/delete');
		
		//分页
		$config['base_url'] 			= site_url('product/manufacturer');
		$config['num_links'] 			= 2;
	...
	{
		if (!$this->user->hasPermission('modify', 'sale/product/manufacturer')) {

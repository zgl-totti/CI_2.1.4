...
//		$info	=	xss_clean($info);
	...
	*/
	...
	public function useropenid($openid = '',$did = ''){
		if ( !$openid ) {
			Return false;;
		}

		$where		=	array('openid'=>$openid,'did'=>$did);
		$this->db->select('*');
		$memberinfo	=	$this->db->limit(1)->get_where('signup',$where)->row_array();

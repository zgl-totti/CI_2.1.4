...
class Assess_model extends CI_Model {
	...
	* @return		
	*/
	public function get_all_times($where = ''){
		if ( !$where ) {
			Return false;
		}
		$where	=	xss_clean($where);
		$this->db->select('id,btime,ecog_ps,pname,pnumber,specific_symptoms,days');
		$this->db->order_by('btime','DESC');
		$info	=	$this->db->get_where($this->table_name,$where)->result_array();

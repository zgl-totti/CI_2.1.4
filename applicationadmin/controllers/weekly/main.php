...
class Main extends CI_Controller {
	...
	*/
	public function edit( $sid = '' ){
		if ( isset($_POST['submit']) && $_POST['submit'] ) {
			...
			$info['periods']	=	$post['periods'];
			$info['release']	=	$post['release'];
			//	详细介绍
			$info['desc']		=	htmlspecialchars($post['desc']);
			$info['updatetime']		=	time();
			
			if (isset($_FILES['thumb']) && $_FILES['thumb']['name']) {
				//	详情图片修改处理
				$this->load->library('attachmentclass');
				$this->attachmentclass->upload_dir	=	'weekly';
	...
	*/
	public function search(){
		...
		$info	=	$this->Weekly_model->search($keywords,$page,$pagesize);
		
		$url	= '/weekly/main/search?q='.$keywords;
		$pages	=	pagesadmin($info['total'],$pagesize,'',$url);
		
		$this->config->set_item('enable_query_strings',false);
		$info['keywords']	=	$keywords;
		$info['pages']		=	$pages;
		templates('weekly','search',$info);
		exit;
	}

...
class Main extends CI_Controller {
	...
	*/
	public function index($page = ''){
		...
		$page	=	max(1,$page);
		$pagesize	=	10;
		$info	=	$this->Business_model->lists('',$page,$pagesize);
		$pages	=	pages($info['total'],$pagesize,4,'/business/main/index');
		$info['pages']	=	$pages;
		templates('business','index',$info);
		exit;
	}
	...
	*/
	public function edit( $cid = '' ){
		if ( isset($_POST['submit']) && $_POST['submit'] ) {
			...
			$rows	=	$this->Business_model->update($info,array('id'=>$cid));
			if ( $rows ) {
				//	记录后台操作日志
				manage_log('card','main','edit','/card/main/edit','修改项目信息',array('id'=>$cid));
			}

			redirect(site_aurl('business/main'));
			exit;
		}else{
			$cid		=	$cid ? intval($cid) : '';
			...
			$where	=	array('id'=>$cid);
			$data['info']	=	$this->Business_model->getone($where);

			//	获取用户组详细信息
			templates('business','edit',$data);
		}
	}
	...
	*/
	public function deletes(){
		if ( isset($_POST['submit']) && $_POST['submit'] ) {
            ...
            if ( !$idArr ) {
                redirect(site_url('business/main/index'));
                exit;
            }

            $deletes	=	$this->Business_model->deletes($idArr);
            if ( $deletes ) {
                //	记录后台操作日志
                manage_log('business','main','deletes','/business/main/deletes','删除项目',json_encode($idArr));
            }
            redirect(site_url('business/main/index'));
            exit;
		}
	...
	*/
	public function search(){
		...
		$info	=	$this->Business_model->search($keywords,$page,$pagesize);
		$url	= '/business/main/search?q='.$keywords;

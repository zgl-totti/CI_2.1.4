	...
	*/
	public function edit( $cid = '' ){
		if ( isset($_POST['submit']) && $_POST['submit'] ) {
			...
			$rows	=	$this->posapply_model->update($info,array('id'=>$cid));
			if ( $rows ) {
				//	记录后台操作日志
				manage_log('posapply','main','edit','/posapply/main/edit','修改项目信息',array('id'=>$cid));
			}
			redirect(site_aurl('posapply/main'));
			exit;
		}else{
			$cid=$cid ? intval($cid) : '';
			...
			$where=array('id'=>$cid);
			$data['info']=$this->posapply_model->getone($where);
			//	获取用户组详细信息
			templates('posapply','edit',$data);
		}
	}
	...
	public function del(){
		if ( isset($_POST['submit']) && $_POST['submit'] ) {
			...
			if ( !$idArr ) {
				redirect(site_url('posapply/main/index'));
				exit;
			}
			$deletes=$this->posapply_model->deletes($idArr);
			if ( $deletes ) {
				//	记录后台操作日志
				manage_log('posapply','main','del','/posapply/main/del','删除项目',json_encode($idArr));
			}
			redirect(site_url('posapply/main/index'));
			exit;
		}
	...
	public function search(){
		...
		$info	=	$this->posapply_model->search($keywords,$page,$pagesize);
		$url	= '/posapply/main/search?q='.$keywords;
	...
	public function postdata($post){
		...
		$info['businessLicense']=$post['businessLicense'] ;
